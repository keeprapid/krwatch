...
#		logger.debug("ModuleControl::__init__")
        ...
        self.filenamedict['alarmcenter'] = "alarmcenter_worker.py"
        self.filenamedict['datacenter'] = "datacenter_worker.py"
        self.filenamedict['devicelogic'] = "devicelogic_worker.py"
        self.filenamedict['gearcenter'] = "gearcenter_worker.py"
        self.filenamedict['httpproxy'] = "httpproxy_worker.py"
        self.filenamedict['lbs_dog'] = "lbsdog_worker.py"
        self.filenamedict['lbs_proxy'] = "lbs_proxy_worker.py"
        self.filenamedict['member'] = "member_worker.py"
        self.filenamedict['member_dog'] = "memberdog_worker.py"
        self.filenamedict['mtpdatacenter'] = "mtpdatacenter_worker.py"
        self.filenamedict['mtpdeviceproxy'] = "mtpdeviceproxy_worker.py"
        self.filenamedict['notify'] = "notify_worker.py"
        self.filenamedict['session'] = "session_worker.py"
    ...
    def startapp(self, appname):
        ...
        if appname not in self.filenamedict:
            return

        launchfile = self.filenamedict.get(appname)
        mode = '1'
        ...
        cmdstr = ''
        if mode == '1':
            if appname in ['anps_proxy','apns_oda']:
                cmdstr = 'python2.7 %s/apps/%s/%s' % (os.getcwd(),appname, launchfile)
            else:
                cmdstr = 'python %s/apps/%s/%s' % (os.getcwd(), appname, launchfile)
        else:
            return

        # logger.debug(cmdstr)
        # logger.debug("start---%s----"%(appname))
    ...
    def _startconsumer(self):
        logger.debug("Start ModuleControl consumer")
