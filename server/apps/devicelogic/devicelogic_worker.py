...
class DeviceLogic(threading.Thread, workers.WorkerBase):
    ...
    def __init__(self, thread_index):
        ...
        if 'devicelogic' in self._config:
            if 'Consumer_Queue_Name' in self._config['devicelogic']:
                self.recv_queue_name = self._config['devicelogic']['Consumer_Queue_Name']
...
#        self.gearconn = pymongo.Connection(self._json_dbcfg['mongo_ip'],int(self._json_dbcfg['mongo_port']))
        ...
        self.geardb = self.gearconn.zhijian_device
        self.collect_gearinfo = self.geardb.devices
...
#            imeikey = resultlist[0]
    ...
    def _proc_gps(self, cmdbody):
        ...
        imei_str = cmdlist[0]
        imei = int(imei_str)
#        searchkey = self.KEY_IMEI_MNAME_MID % (imei_str, '*', '*')
#        resultlist = self._redis.keys(searchkey)
#        if len(resultlist) == 0:
#            logger.error("no need record, device out of service")
#            return
#
#        imeikey = resultlist[0]
#        memberid = imeikey.split(':')[4]
...
#                imeikey = self.KEY_IMEI_ID % (imei_str,imei_id)
#                for key in imeiinfo:
#                    if key in ['_id','createtime','activetime']:
#                        imeiinfo[key] = imeiinfo[key].__str__()
#
#                self._redis.hmset(imeikey, imeiinfo)
#                logger.error("get back from db imei:%s"%(imeikey))
...
#        memberinfo = self.collect_memberinfo.find_one({'_id':ObjectId(memberid)})
#        if memberinfo is None:
#            logger.error("ERRORCODE_MEMBER_NOT_EXIST")
#            return
