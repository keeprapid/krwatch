...
#            logger.debug('action_body : %s' % (action_body))
        ...
        elif action_cmd == 'geo_location':
            self._proc_action_geo_location(action_version, action_body, msg_out_head, msg_out_body)
        else:
            ...
            if datajson.get('status') and datajson.get('info'):
                if datajson.get('status') == '1' and datajson.get('info') == 'OK':
                    ...
                    if result is not None:
                        ...
                        if result.get('province') is not None:
                            province = result.get('province')
                        if result.get('city') is not None:
                            city = result.get('city')
                        if result.get('citycode') is not None:
                            citycode = result.get('citycode')
                        if result.get('adcode') is not None:
                            adcode = result.get('adcode')
                        if result.get('road') is not None:
                            township = result.get('road')
                        if result.get('district') is not None:
...
#                        logger.debug('convert baidu 2 wgs = %s' % wgs.__str__())
                        ...
                        wgslat = wgs[1]
                        updatelbsinfo = dict()
                        updatelbsinfo['latitude'] = wgslat
                        updatelbsinfo['longitude'] = wgslong
                        updatelbsinfo['accuracy'] = acc
                        updatelbsinfo['address'] = address
                        updatelbsinfo['country'] = country
                        updatelbsinfo['province'] = province
                        updatelbsinfo['city'] = city
                        updatelbsinfo['citycode'] = citycode
                        updatelbsinfo['adcode'] = adcode
                        updatelbsinfo['township'] = township
                        updatelbsinfo['towncode'] = towncode
                        updatelbsinfo['district'] = district

                        self.collect_lbsinfo.update_one({'_id':ObjectId(lbs_objectid)},{'$set':updatelbsinfo})
                        insertdict = dict()
                        ...
                        insertdict['username'] = current_lbs_info.get('username')
                        insertdict['nation'] = current_lbs_info.get('nation')
                        insertdict['nickname'] = current_lbs_info.get('nickname')
                        insertdict['codetype'] = current_lbs_info.get('codetype')
                        insertdict['passdate'] = current_lbs_info.get('passdate')
                        insertdict['passland'] = current_lbs_info.get('passland')
                        insertdict['devicetype'] = current_lbs_info.get('devicetype')
                        insertdict['deviceid'] = current_lbs_info.get('deviceid')    
                        insertdict['memberid'] = current_lbs_info.get('memberid')
                        ...
                        updatedict['last_location_time'] = current_lbs_info.get('timestamp')
                        self.collect_gearinfo.update_one({'imei':imei},{'$set':updatedict})
                        self.collect_memberinfo.update_one({'_id':ObjectId(current_lbs_info.get('memberid'))},{'$set':updatedict})
                        #增加log
                        insertlog = dict()
                        insertlog['imei'] = imei
                        insertlog['url'] = url
                        ...
                        #检查围栏
                        if current_lbs_info.get('alarm_enable') == '1':
                            body = dict()
                            body['imei'] = imei
                            body['latitude'] = wgslat
                            body['longitude'] = wgslong
                            body['imeikey'] = imeikey
                            body['memberid'] = current_lbs_info.get('memberid')
                            action = dict()
                            action['body'] = body
                            action['version'] = '1.0'
    ...
    def _proc_action_geo_location(self, version, action_body, retdict, retbody):
        '''
        input : {    'action_cmd'  : '_proc_action_geo_location', M
                     'seq_id      : M
                     'version'    : M
                     'body'   :{
                body['imei'] = imei
        ...
        '''
        logger.debug(" into _proc_action_geo_location action_body:[%s]:%s"%(version,action_body))
        try:
#        if 1:
            
            if ('lat' not in action_body) or  ('lng' not in action_body) or  ('gps_objectid' not in action_body) or  ('memberid' not in action_body):
                retdict['error_code'] = self.ERRORCODE_CMD_HAS_INVALID_PARAM
                return
            if action_body['lat'] is None or action_body['lat'] == '':
                retdict['error_code'] = self.ERRORCODE_MEMBER_PASSWORD_INVALID
                return
            if action_body['lng'] is None or action_body['lng'] == '':
                retdict['error_code'] = self.ERRORCODE_MEMBER_PASSWORD_INVALID
                return
            if action_body['gps_objectid'] is None or action_body['gps_objectid'] == '':
                retdict['error_code'] = self.ERRORCODE_MEMBER_PASSWORD_INVALID
                return
            ...
            gps_objectid = action_body['gps_objectid']
            lat = action_body['lat']
            lng = action_body['lng']
            memberid = action_body['memberid']


            url = "%skey=%s&location=%s" % (self.GEO_SERVICE_AMAP_URL,self.GEO_SERVICE_AMAP_KEY,"%s,%s"%(lng,lat))
            logger.debug("url = %s" %(url))
            resp = urllib2.urlopen(url,timeout=5)
            # logger.debug(resp)
            data = resp.read()
...
#            logger.debug(datajson)
            ...
            if datajson.get('status') and datajson.get('info'):
                if datajson.get('status') == '1' and datajson.get('info') == 'OK':
                    result = datajson.get('regeocode')
                    if result is not None:
                        if 'formatted_address' in result:
                            updatedict['address'] = result['formatted_address']
                        if 'addressComponent' in result:
                            addressComponent = result['addressComponent']
                            needkeylist = ['country','province','city','citycode','district','adcode','township','towncode']
                            for key in needkeylist:
                                if key in addressComponent and isinstance(addressComponent[key],list) is False:
                                    updatedict[key] = addressComponent[key]
                                else:
                                    updatedict[key] = ''

            logger.debug(updatedict)
            if len(updatedict)>0:
                self.collect_gpsinfo.update_one({'_id':ObjectId(gps_objectid)},{'$set':updatedict})
