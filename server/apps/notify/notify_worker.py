...
#            logger.debug('action_body : %s' % (action_body))
    ...
    def _sendEmail(self, dest, content, email_from, email_subject):
        fs = open("/opt/Keeprapid/KRWatch/server/conf/email.conf", "r")
...
#        if 1:
            ...
            vcode = '%.6d' % (random.SystemRandom().randint(0,100000))
            logger.debug(vcode)
...
#            if 1:
            ...
            #通知ios客户端：
            if 'ios_token' in userprofile and userprofile['ios_token'] != '' and userprofile['ios_token'] is not None:
...
#                                content = templateinfo[notify_id]['chs']['content']
                    ...
                    if len(tokenlist):
                        if 'anps_proxy' in self._config:
                            ...
                            action['version'] = '1.0'
                            action['seq_id'] = random.SystemRandom().randint(0,9999999)
                            action['from'] = ""
